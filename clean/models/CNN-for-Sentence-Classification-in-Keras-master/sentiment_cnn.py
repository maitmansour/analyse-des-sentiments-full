...
labels_index=[0,1]
texts_big_array = open('data/to_predict.txt').read().split("\n")
texts_array=list(chunks(texts_big_array, 1000))

with open('data/predection.txt', 'a') as f:
    for texts in texts_array:
        tk = Tokenizer()
        tk.fit_on_texts(texts)
        index_list = tk.texts_to_sequences(texts)
        x_train = pad_sequences(index_list, maxlen=sequence_length)
        predictions = model.predict(x_train)
        print(predictions)
        t = 0
        for text in texts:
            print("Prediction for \"%s\": " % (text))
            print("\t%s ==> %f" % (t, predictions[t]))
            if predictions[t] < 0.70:
                polarity="negatif"
            elif predictions[t] >= 0.70 and predictions[t] <= 0.80:
                polarity="positif"
            else:
                polarity="positif"
            f.write("%s\t%s\n" % (text,polarity))
